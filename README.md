...
Тут описан сбор правил по стилю кода и архитектуре Angular для любого проекта. Цель — описать минимум лучших практик,
чтобы новый программист без его обучения писал "правильный" код и архитектуру. В конечном счете это нужно для соблюдения
принципа - "Нормально делай, нормально будет". Приветствуются предложения по улучшению данного гайда.
...
5. Создаем провайдер `LoggerServiceProvider` в отдельном файле для каждого
   ...
   . Прописываем в providers условия выбора Production или Mock, например`LoggerServiceProvider`. Используем `useValue`
   или `useFactory`
...
Используем стратегию `OnPush` у всех компонентов.
...
2. Сущность - то что хранится в store `Data`
...
Начинаем называть `actions` так же как и функции с глагола. Плохой пример:
